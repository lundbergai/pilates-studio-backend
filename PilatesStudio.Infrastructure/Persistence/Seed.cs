...
namespace PilatesStudio.Infrastructure.Persistence
{
    public static class Seed
    {
        ...
        public static void ApplyScheduledClassesSeed(PilatesDbContext context)
        {
            ...
            var instructors = context.Users.Where(u => u.IsInstructor).OrderBy(u => u.Id).ToList();
            ...
            {
                new ScheduledClass { ClassTypeId = classTypes[0].Id, InstructorId = instructors[0].Id, StartTime = now.AddDays(1).AddHours(8), BookedSpots = 0, CreatedAt = now, UpdatedAt = now },
                new ScheduledClass { ClassTypeId = classTypes[1].Id, InstructorId = instructors[0].Id, StartTime = now.AddDays(1).AddHours(10), BookedSpots = 3, CreatedAt = now, UpdatedAt = now },
                new ScheduledClass { ClassTypeId = classTypes[2].Id, InstructorId = instructors[1 % instructors.Count].Id, StartTime = now.AddDays(1).AddHours(14), BookedSpots = 5, CreatedAt = now, UpdatedAt = now },
                new ScheduledClass { ClassTypeId = classTypes[3].Id, InstructorId = instructors[1 % instructors.Count].Id, StartTime = now.AddDays(2).AddHours(9), BookedSpots = 2, CreatedAt = now, UpdatedAt = now },
                new ScheduledClass { ClassTypeId = classTypes[0].Id, InstructorId = instructors[0].Id, StartTime = now.AddDays(2).AddHours(16), BookedSpots = 8, CreatedAt = now, UpdatedAt = now },
                new ScheduledClass { ClassTypeId = classTypes[1].Id, InstructorId = instructors[1 % instructors.Count].Id, StartTime = now.AddDays(3).AddHours(11), BookedSpots = 0, CreatedAt = now, UpdatedAt = now }
            };
